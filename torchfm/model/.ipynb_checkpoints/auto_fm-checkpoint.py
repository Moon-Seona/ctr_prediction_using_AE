...
class RecurrentAutoencoder(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        return self.linear(output2), h[-1], output
...
class FactorizationMachineModel(torch.nn.Module):
    ...
    def __init__(self, field_dims, vocab_size, embed_dim):
        ...
        self.encoder_linear2 = torch.nn.Linear(embed_dim, embed_dim)

        # when use lstm
        self.embed_output_dim = (len(field_dims)+1) * embed_dim # all of them +3
    ...
    def forward(self, x, additional, column): #
        ...
        if column == 'appbundle' : 
            ...
            #encoded = self.encoder_linear2(F.relu(self.encoder_linear(hidden.view(-1,1, self.ae_dim))))
            feat_embedding = torch.cat([feat_embedding[:,:1,:], encoded, feat_embedding[:,1:,:]], dim=1) 
            col_embedding = feat_embedding[:,1,:]
        elif column == 'carrier' : 
            pred_seq, hidden, output = self.autoencoder2(additional[:,149:199])
            encoded = self.encoder_linear(hidden.view(-1,1, self.ae_dim))
            #encoded = self.encoder_linear2(F.relu(self.encoder_linear(hidden.view(-1,1, self.ae_dim))))
            feat_embedding = torch.cat([feat_embedding[:,:1,:], encoded, feat_embedding[:,1:,:]], dim=1)
            col_embedding = feat_embedding[:,1,:]
            ...
            encoded = self.encoder_linear(hidden.view(-1,1, self.ae_dim))
            #encoded = self.encoder_linear2(F.relu(self.encoder_linear(hidden.view(-1,1, self.ae_dim))))
            #encoded = self.encoder_linear2(F.relu(self.encoder_linear2(F.relu(self.encoder_linear(hidden.view(-1,1, self.ae_dim))))))
        ...
        return torch.sigmoid(x.squeeze(1)), pred_seq, hidden, encoded.squeeze(1), col_embedding, hidden
