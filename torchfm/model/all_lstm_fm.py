...
class FactorizationMachineModel(torch.nn.Module):
    ...
    def __init__(self, field_dims, vocab_size, embed_dim):
        ...
        self.ae_dim = 32 # 256 # autoencoder embedding dim
        ...
        self.autoencoder2 = RecurrentAutoencoder(50, vocab_size, self.ae_dim) # carrier +1
        self.autoencoder3 = RecurrentAutoencoder(38, vocab_size, self.ae_dim) # carrier +1

        self.encoder_linear = torch.nn.Linear(self.ae_dim, embed_dim)

        # when use lstm
        self.embed_output_dim = (len(field_dims)+3) * embed_dim # all of them +3
    ...
    def forward(self, x, additional, column):
        ...
        pred_seq, hidden = self.autoencoder(additional[:,:149]) # app bundle
        pred_seq2, hidden2 = self.autoencoder2(additional[:,149:199]) # carrier
        pred_seq3, hidden3 = self.autoencoder3(additional[:,199:]) # make
        
        encoded = self.encoder_linear(hidden.view(-1,1,self.ae_dim)) # app bundle
        encoded2 = self.encoder_linear(hidden2.view(-1,1,self.ae_dim)) # carrier
        encoded3 = self.encoder_linear(hidden3.view(-1,1,self.ae_dim)) # make
        
        #total
        feat_embedding = torch.cat([feat_embedding[:,:1,:], encoded, encoded2, feat_embedding[:,1:2,:], encoded3, feat_embedding[:,2:,:]], dim=1) 
